"""
Nested-object and reference-identity rules for the pure PUT merge.

Relocated from tests/test_client_merge.py, whose docstring promises tests
of the client's GET-merge-PUT wiring: these call merge_put_data directly,
with no client, no transport and no mocks. They live apart from
tests/test_merge.py only to keep both files under the 500-line limit.
"""

from src.gramps_mcp.merge import merge_put_data


class TestNestedAndIdentityRules:
    """Rules governing nested objects and reference-entry identity."""
