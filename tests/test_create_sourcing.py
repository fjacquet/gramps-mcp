...
class TestCreateRepositoryTool:
    ...
    @pytest.mark.asyncio
    async def test_create_repository_success(self, gramps_client, tree_id, note_handle):
        ...
        try:
            ...
            repository_handle = handle_on_line(
                text, "National Archives - Boston Branch"
            )
        finally:
...
class TestCreateSourceTool:
    ...
    async def test_create_source_success(
        self, gramps_client, tree_id, repository_handle, media_handle, note_handle
    ):
        ...
        try:
            ...
            source_handle = handle_on_line(text, "Birth Register 1850-1860")
        finally:
            if source_handle:
                await delete_entity(
    ...
    async def test_create_source_with_media_path(
        ...
        source_handle = media_handle = None
        try:
            result = await create_source_tool(
                {
...
class TestCreateCitationTool:
    ...
    async def test_create_citation_success(
        self, gramps_client, tree_id, source_handle, media_handle, note_handle
    ):
        ...
        try:
            ...
            citation_handle = handle_on_line(text, "Page 45, Entry 23")
        finally:
            if citation_handle:
                await delete_entity(
    ...
    async def test_create_citation_with_media_path(
        ...
        try:
            ...
            # Reason: only the inline-uploaded media is this test's own
            # leftover - the other ref is the module-scoped media_handle
            # fixture, which conftest.py already tears down; deleting it
            # again here would just be a harmless no-op via delete_entity,
            # but naming it explicitly keeps the intent clear.
            inline_media_handle = next(
                (ref["ref"] for ref in media_refs if ref["ref"] != media_handle),
                None,
            )
        finally:
            if citation_handle:
